...
Each child still uses its own fade-in animation, but the parent adds a short delay before starting the next one, so the cards appear left to right (or top to bottom) instead of all at once. The small stat cards in the About Me section use the same trick.
...
The project cards and blog post cards have a soft gradient outline that appears and slowly spins when you hover over them. That one's actually not Framer Motion at all. It's plain CSS, using a class called `gradient-border`:
